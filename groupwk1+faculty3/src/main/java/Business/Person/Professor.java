...
public class Professor extends Person {
    ...
    public static Professor loadFromDatabase(Connection connection, String _personID, String _username) throws SQLException {
//        Person person = Person.loadFromDatabase(connection, personID);
//        if (person == null) {
//            return null;
//        }

        String selectProfessorQuery = "SELECT Professor.*, person.* FROM Professor left join person on Professor.id = person.PersonID WHERE person.PersonID = ? or Professor.username = ?";
        try (PreparedStatement statement = connection.prepareStatement(selectProfessorQuery)) {
            statement.setString(1, _personID);
            ...
            if (resultSet.next()) {
                ...
                Professor professor = new Professor(personName, personID, username, "", enabled, role);
